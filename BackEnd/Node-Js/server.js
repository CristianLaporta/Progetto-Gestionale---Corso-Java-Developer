...
app.post('/log', (req, res) =>{
    ...
    connection.query("SELECT * FROM `ultimeoperazioni`ORDER BY id DESC ", function (err, risposta){
        if(err){
            console.log("ci sono degli errori nella ricerca degli utenti. errore: "+err)
        }
        res.json(risposta)
...
app.post('/login', (req, res) =>{
    ...
    connection.query("SELECT * FROM utenti WHERE email = '"+req.body.email+"' AND password = '"+req.body.password+"' ", function (err, risposta){
      //se si crea un errore, allora stampa in console un messaggio
        if(err){
            console.log("ci sono degli errori nella ricerca degli utenti. errore: "+err)
        }
       ...
       connection.query("UPDATE `utenti` SET `token` = '"+token+"' WHERE `utenti`.`email` = '"+req.body.email+"'", function (err,risposta2){
        log(undefined,"login",risposta[0].nome+" "+risposta[0].cognome);
       res.json(token)
       }) }else{
       res.json([]);
           }
...
app.post('/user', (req, res) =>{
    ...
    //creo una richiesta al database inserendo una query che mi cerca ciò che voglio
connection.query("SELECT * FROM `utenti`ORDER BY id DESC ", function (err, risposta){
    if(err){
        console.log("ci sono degli errori nella ricerca degli utenti. errore: "+err)
    }
risposta[0].password = undefined;
...
app.post('/prodotti', (req, res) =>{
    ...
    //creo una richiesta al database inserendo una query che mi cerca ciò che voglio
connection.query("SELECT * FROM `movimenti` ORDER BY id DESC ", function (err, risposta2){
    if(err){
        console.log("ci sono degli errori nella ricerca degli utenti. errore: "+err)
    }
    res.json(risposta2)
...
if(risposta2[0].admin== "true" ){
        //creo una richiesta al database inserendo una query che mi cerca ciò che voglio
connection.query("DELETE FROM utenti WHERE `utenti`.`id` = "+req.body.idutenteel, function (err, risposta1){
    if(err){
        console.log("ci sono degli errori nella ricerca degli utenti. errore: "+err)
    }
    
    log(req.body.idutenteel,"eliminatoutente",risposta2[0].nome+" "+risposta2[0].cognome);
    res.json(true);
    })
}else{
    res.json(false);
...
app.delete('/eliminaprodotto', (req, res) =>{
    ...
    connection.query("SELECT * FROM utenti WHERE email = '"+req.body.email+"' AND token = '"+req.body.token+"' ", function (err, risposta2){
        //se si crea un errore, allora stampa in console un messaggio
         ...
         connection.query("SELECT * FROM `movimenti` WHERE id = "+req.body.idelimina,function (err, risposta3){
         if(risposta3.length !=0){
            if(risposta3[0].creatore == req.body.email || risposta2[0].admin == "true"){
                connection.query("DELETE FROM movimenti WHERE `movimenti`.`id` = "+risposta3[0].id, function (err, risposta4){
                    log(risposta3[0].descrizione,"eliminatomovimento",risposta2[0].nome+" "+risposta2[0].cognome);
                    res.json(true)
                   });
            }else{
               res.json(false);
...
app.listen(port, () =>{
console.log("ho startato correttamente il tuo server node js con la porta "+port)
});
...
connection.query("INSERT INTO `ultimeoperazioni` (`id`, `dataora`, `log`) VALUES (NULL, current_timestamp(), '"+testop+"');", function (err, risposta){
        //se si crea un errore, allora stampa in console un messaggio
          if(err){
              console.log("ci sono degli errori nella ricerca degli utenti. errore: "+err)
          }
